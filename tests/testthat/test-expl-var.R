...
test_that("getVarianceExplained handles sparse inputs", {
    ...
    library(Matrix)
    counts(normed_sparse) <- as(as(as(counts(normed), "dMatrix"), "generalMatrix"), "CsparseMatrix")
    logcounts(normed_sparse) <- as(as(as(logcounts(normed), "dMatrix"), "generalMatrix"), "CsparseMatrix")
