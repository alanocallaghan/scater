...
#'
#' \pkg{scater} functions that plot points share a number of visualization
#' parameters, which are described on this page.
#'
#' @section Aesthetic parameters:
#' \describe{
#' \item{\code{add_legend}:}{Logical scalar, specifying whether a legend should
#' be shown.
#' Defaults to TRUE.}
#' \item{\code{theme_size}:}{Integer scalar, specifying the font size.
#' Defaults to 10.}
#' \item{\code{point_alpha}:}{Numeric scalar in [0, 1], specifying the
#' transparency.
#' Defaults to 0.6.}
#' \item{\code{point_size}:}{Numeric scalar, specifying the size of the points.
#' Defaults to \code{NULL}.}
#' \item{\code{jitter_type}:}{String to define how points are to be jittered in
#' a violin plot.
#' This is either with random jitter on the x-axis (\code{"jitter"}) or in a
#' \dQuote{beeswarm} style (if \code{"swarm"}, default).
#' The latter usually looks more attractive, but for datasets with a large
#' number of cells, or for dense plots, the jitter option may work better.}
#' }
#'
#' @section Distributional calculations:
#' \describe{
#' \item{\code{show_median}:}{Logical, should the median of the distribution
#' be shown for violin plots?
#' Defaults to \code{FALSE}.}
#' \item{\code{show_violin}:}{Logical, should the outline of a violin plot be
#' shown?
#' Defaults to \code{TRUE}.}
#' \item{\code{show_smooth}:}{Logical, should a smoother be fitted to a scatter
#' plot?
#' Defaults to \code{FALSE}.}
#' \item{\code{show_se}:}{Logical, should standard errors for the fitted line
#' be shown on a scatter plot when \code{show_smooth=TRUE}?
#' Defaults to \code{TRUE}.}
#' }
#'
#' @section Miscellaneous fields:
#' Addititional fields can be added to the data.frame passed to \link{ggplot}
#' by setting the \code{other_fields} argument.
#' This allows users to easily incorporate additional metadata for use in
#' further \pkg{ggplot} operations.
#'
#' The \code{other_fields} argument should be character vector where each string
#' is passed to \code{\link{retrieveCellInfo}} (for cell-based plots) or
#' \code{\link{retrieveFeatureInfo}} (for feature-based plots).
#' Alternatively, \code{other_fields} can be a named list where each element is
#' of any type accepted by \code{\link{retrieveCellInfo}} or
#' \code{\link{retrieveFeatureInfo}}.
#' This includes \link{AsIs}-wrapped vectors, data.frames or
#' \linkS4class{DataFrame}s.
#'
...
#' \code{\link{retrieveFeatureInfo}}.
#' If these clash with inbuilt names
#' (e.g., \code{X}, \code{Y}, \code{colour_by}), a warning will be raised
#' and the additional column will not be added to avoid overwriting an existing
#' column.
#'
...
#' @importFrom ggplot2 ggplot geom_violin aes_string xlab ylab stat_summary
#'  geom_jitter position_jitter coord_flip geom_point stat_smooth geom_tile
#'  theme_bw theme
.central_plotter <- function(
        object, xlab = NULL, ylab = NULL, colour_by = NULL, shape_by = NULL,
        size_by = NULL, fill_by = NULL, show_median = FALSE, show_violin = TRUE,
        show_smooth = FALSE, show_se = TRUE, theme_size = 10, point_alpha = 0.6,
        point_size = NULL, add_legend = TRUE, point_FUN = NULL,
        jitter_type = "swarm", rasterise = FALSE) {
...
# Creates either a scatter plot, (horizontal) violin plots, or a rectangle plot.
    if (is.numeric(object$Y) != is.numeric(object$X)) {
        ...
        if (show_violin) {
            ...
            plot_out <- plot_out + do.call(geom_violin, c(viol_args,
                list(colour = "gray60", alpha = 0.2, scale = "width", width = 0.8))
            )
        }
        ...
        point_out <- .get_point_args(
            colour_by, shape_by, size_by, alpha = point_alpha,
            size = point_size, fill_by = fill_by
        )
        if (is.null(point_FUN)) {
            if (jitter_type == "swarm") {
                point_FUN <- function(...) {
                    geom_quasirandom(..., width = 0.25, groupOnX = TRUE,
                        bandwidth = 1, dodge.width = 0.75
                    )
                }
            } else {
                point_FUN <- function(...) {
        ...
        if (!is.null(fill_by)) {
            plot_out <- .resolve_plot_colours(
                plot_out, object$fill_by, fill_by, fill = TRUE
            )
        }
        ...
        plot_out <- plot_out +
            geom_tile(
                aes_string(x = "X", y = "Y", height = "2*YWidth", width = "2*XWidth"),
...
.get_point_args <- function(
        colour_by, shape_by, size_by, fill_by = NULL, alpha = 0.65, size = NULL
    ) {
    ...
    # set fill when there is no fill colour, to distinguish between e.g., pch=16 and pch=21.
    if (is.null(colour_by)) {
