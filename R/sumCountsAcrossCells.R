...
#' to use to store the number of cells in each group.
...
#' Columns are ordered by \code{levels(ids)} and the number of cells per level is reported in the \code{"ncells"} attribute.
#'
#' For \code{sumCountsAcrossCells} with a DataFrame \code{ids}, a numeric matrix of summed/averaged values is similarly returned.
#' Each column corresponds to a unique combination of levels in \code{ids}, the identities of which are reported in the \code{"ids"} attribute.
#' The columns themselves are unnamed but are sorted according to the ordering of the combinations. 
#' The number of cells per combination is reported in the \code{"ncells"} attribute.
#'
...
.sum_counts_across_cells <- function(x, ids, subset_row=NULL, subset_col=NULL, 
    average=FALSE, BPPARAM=SerialParam()) 
{
    ...
    average=FALSE, BPPARAM=SerialParam(), modifier=NULL) 
{
    ...
    if (multi) {
        ...
        m <- match(cn, ids)
        attr(out, "ids") <- coldata[m,,drop=FALSE]
        colnames(out) <- NULL
    }
