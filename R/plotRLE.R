...
plotRLE <- function(object, exprs_values="logcounts", exprs_logged = TRUE, 
    ...
    if (style == "full") {
        ...
        aesth <- aes(x = x, group = x, y = rle, colour = .data[[colour_lab]], fill = .data[[colour_lab]])
...
.plotRLE_minimal <- function(df, colour_by, ncol) {
    plot_out <- ggplot(df, aes(x = x, fill = .data[[colour_by]])) +
        geom_segment(aes(xend = x, y = q25, yend = q75), colour = "gray60") +
        geom_segment(aes(xend = x, y = q75, yend = whiskMax, colour = .data[[colour_by]])) +
        geom_segment(aes(xend = x, y = q25, yend = whiskMin, colour = .data[[colour_by]])) +
        geom_point(aes(y = q50), shape = 21) +
