...
aggregateAcrossFeatures <- function(x, ids, ..., use.assay.types="counts") {
    collected <- list()
...
.choose_assay_names <- function(x, assay.types) {
    if (is.numeric(assay.types)) {
        assayNames(x)[assay.types]
    } else {
        assay.types
    }
}
