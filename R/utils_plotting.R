.incorporate_common_vis_row <- function(
        df, se, mode, colour_by, size_by, shape_by,
        by_exprs_values, by_show_single, other_fields, multiplier = NULL) {
    colour_by_out <- retrieveFeatureInfo(
        se, colour_by, exprs_values = by_exprs_values
    )
    ...
    shape_by_out <- retrieveFeatureInfo(
        se, shape_by, exprs_values = by_exprs_values
    )
    ...
    size_by_out <- retrieveFeatureInfo(
        se, size_by, exprs_values = by_exprs_values
    )
...
.incorporate_common_vis_col <- function(
        df, se, mode, colour_by, size_by, shape_by, fill_by, order_by,
        by_exprs_values, by_show_single, other_fields,
        multiplier = NULL, swap_rownames = NULL) {
    colour_by_out <- retrieveCellInfo(
        se, colour_by, exprs_values = by_exprs_values,
    ...
    fill_by_out <- retrieveCellInfo(se, fill_by, exprs_values = by_exprs_values,
        swap_rownames = swap_rownames)
    fill_by <- fill_by_out$name
    if (!is.null(multiplier)) {
        fill_by_out$value <- fill_by_out$value[multiplier]
    }
    df$fill_by <- fill_by_out$value

    shape_by_out <- retrieveCellInfo(se, shape_by,
    ...
    order_by_out <- retrieveCellInfo(se, order_by,
        exprs_values = by_exprs_values, swap_rownames = swap_rownames)
    ...
    list(
        df = df, colour_by = colour_by, shape_by = shape_by,
        size_by = size_by, order_by = order_by
    )
}
...
# https://gastonsanchez.wordpress.com/2012/08/27/scatterplot-matrices-with-ggplot/
    ...
    exp_grid <- expand.grid(
        x = seq_len(ncol(data_matrix)),
        y = seq_len(ncol(data_matrix))
    )
    ...
    all_panels <- do.call("rbind", lapply(seq_len(nrow(exp_grid)), function(i) {
        ...
        ycol <- exp_grid[i, "y"]
        data.frame(
            xvar = names(data_matrix)[ycol], yvar = names(data_matrix)[xcol],
    ...
    densities <- do.call("rbind",
        lapply(seq_len(ncol(data_matrix)),
            function(i) {
                data.frame(
                    xvar = names(data_matrix)[i], yvar = names(data_matrix)[i],
                    x = data_matrix[, i]
                )
            }
        )
...
.handle_features <- function(features, object) {
    if (is.logical(features)) {
        ...
        if (any(features > nrow(object)) ||
            any(features <= 0) ||
            any(features != round(features))) {
