...
The `makePerCellDF()` function will intelligently retrieve requested fields from the `colData()`, `assays()`, `altExps()` or `reducedDims()` to create a single `data.frame` for immediate use.
In the example below, we create boxplots of _Snap25_ expression stratified by cell type and tissue of origin:

```{r, fig.wide=TRUE}
ggplot(makePerCellDF(example_sce, 
...
```{r}
ggplot(makePerCellDF(example_sce, "Actb", 
...
Similar operations can be performed on the features using the `makePerFeatureDF()` function,
...
```{r}
ggplot(makePerFeatureDF(example_sce, "featureType", 
