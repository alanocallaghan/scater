...
* `subsets$X$percent`: percentage of all counts that come from the feature control set named `X`.

```{r}
per.cell <- perCellQCMetrics(example_sce, subsets=list(Mito=1:10))
summary(per.cell$sum)
summary(per.cell$detected)
summary(per.cell$subsets$Mito$percent)
```
...
plotColData(example_sce, x = data.frame(sum=per.cell$sum),
    y = data.frame(pct_mito=per.cell$subsets$Mito$percent), 
...
* `subsets$Y$ratio`: ratio of mean counts between the cell control set named Y and all cells.

```{r}
per.feat <- perFeatureQCMetrics(example_sce, subsets=list(Empty=1:10))
summary(per.feat$mean)
summary(per.feat$detected)
summary(per.feat$subsets$Empty$ratio)
```
...
chosen.genes <- 1:100
example_sce2 <- runPCA(example_sce, subset.row=chosen.genes)
...
set.seed(1000)
example_sce <- runTSNE(example_sce, perplexity=10, use.dimred="PCA", n.dimred=10)
