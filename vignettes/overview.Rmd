...
The `ggsce()` function will intelligently retrieve requested fields from the `colData()`, `assays()`, `altExps()` or `reducedDims()` to create a single `data.frame` for immediate use.
...
```{r}
ggsce(example_sce, mapping=aes(x=TSNE.1, y=TSNE.2)) +
...
```{r}
ggsce(example_sce, mapping=aes(x=sizeFactor, y=Actb)) +
    geom_point() +
    geom_smooth()
```
