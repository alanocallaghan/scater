...
The `perCellDataFrameFromSCE()` function will intelligently retrieve requested fields from the `colData()`, `assays()`, `altExps()` or `reducedDims()` to create a single `data.frame` for immediate use.
In the example below, we create boxplots of _Snap25_ expression stratified by cell type and tissue of origin:

```{r, fig.wide=TRUE}
ggplot(perCellDataFrameFromSCE(example_sce, 
        "level1class", "Snap25", "tissue")) +
    geom_boxplot(mapping=aes(x=level1class, y=Snap25)) +
    facet_wrap(~tissue)
```
...
```{r}
ggplot(perCellDataFrameFromSCE(example_sce, 
        "Snap25", "tissue", include_dimred="TSNE"),
    mapping=aes(x=TSNE.1, y=TSNE.2)) +
    geom_point(aes(colour=Snap25)) +
    stat_density_2d() +
    facet_wrap(~tissue) +
...
```{r}
ggplot(perCellDataFrameFromSCE(example_sce, "Actb", 
        include_size_factors=TRUE),
    mapping=aes(x=size_factor, y=Actb)) +
    geom_point() +
    geom_smooth()
```

Similar operations can be performed on the features using the `perFeatureDataFrameFromSCE()` function,
...
```{r}
ggplot(perFeatureDataFrameFromSCE(example_sce, "featureType", 
    MyCell="1772063078_E07")) + 
    geom_violin(mapping=aes(x=featureType, y=MyCell))
```
