...
`SingleCellExperiment` object. `SingleCellExperiment` is a more modern and 
robust class that provides a common data structure used by many single-cell 
Bioconductor packages. Advantages include the capability for on-disk storage of 
data to minimise memory usage for large single-cell datasets.
...
Bioconductor packages. The class requires three input files:
  ...
  captured, etc.)
  3. `featureData`, an `DataFrame` object, where rows are features
...
(no count data required; here we compute counts-per-million from the 
`example_sceset` defined above and transform to the log2 scale) like this:

```{r sceset-make-sceset-exprs-only, eval=FALSE}
...
```{r sceset-add-count-data, eval=FALSE}
...
```{r sceset-demo-replacement, eval=FALSE}
gene_df <- data.frame(Gene = rownames(sc_example_counts))
rownames(gene_df) <- gene_df$Gene
...
```{r plot-expression-theme-bw, eval=FALSE}
